package com.venuyeggadi.problemsolving.leetcode;/* Write a function to delete a node in a singly-linked list.
 ...
 * }
 */
class DeleteNodeInALinkedListSolution1 {
    public void deleteNode(ListNode node) {
        node.val = node.next.val;
        node.next = node.next.next;

        System.gc(); //calling a garbage collector just for fun.
    }
}
/* Here we are just deletint the next node after compying its value into
   current node
*/