# Topic wise questions

#### Refer DSA in Java book, YouTube videos, GFG articles for topics.
#### Use LeetCode discuss, GFG articles for solutions.
#### Curently following [NeetCode](https://neetcode.io/)
#### Later [Seanprasad leetcode patterns](https://seanprashad.com/leetcode-patterns/), [Striver SDE sheet](https://takeuforward.org/interviews/strivers-sde-sheet-top-coding-interview-problems/)

## Arrays and Hashing
  * [Contains Duplicate](https://leetcode.com/problems/contains-duplicate/) - Solution
  * [Valid Anagram](https://leetcode.com/problems/valid-anagram/)
  * [Two Sum](https://leetcode.com/problems/two-sum/) - Solution
  * [Group Anagrams](https://leetcode.com/problems/group-anagrams/) - Solution
  * [Top K Frequent Elements](https://leetcode.com/problems/top-k-frequent-elements/) - Solution
  * [Product of Array Except Self](https://leetcode.com/problems/product-of-array-except-self/) - Solution
  * [Valid Sudoku](https://leetcode.com/problems/valid-sudoku/) - Solution
  * [Encode and Decode Strings](https://leetcode.com/problems/encode-and-decode-strings/) - Solution
  * [Longest Consecutive Sequence](https://leetcode.com/problems/longest-consecutive-sequence/) - Solution



## Searching
## Sorting
## Matrix
## String 
## Hashing
## Bit Masking
## Linked List
## Stack
## Queue
## Heap
## Binary Tree
## Binary Search Tree
## Graph
## Greedy
## Dynamic Programming
## Recursion 
## Algorithms
## Design Patterns

## Math
* ### Easy
    * [Missing Number in Array](https://leetcode.com/problems/missing-number/) - [Solution](./leetcode/268.MissingNumber.java)
    * [Minimum steps to make product equal to one](https://practice.geeksforgeeks.org/problems/minimum-steps-to-make-product-equal-to-one/1) - Solution
    * [Factorial Trailing Zeroes](https://leetcode.com/problems/factorial-trailing-zeroes/) - Solution
    * [Happy Number](https://leetcode.com/problems/happy-number/) - Solution
    * [Plus One](https://leetcode.com/problems/plus-one/) - Solution